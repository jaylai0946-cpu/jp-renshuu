...
寫完整個字就當場評，全部在本機算，**不呼叫 AI**：

1. 使用者的筆畫和範本都放到 KanjiVG 的 109×109
2. 兩邊各重新取樣成 32 個等距點（等距，所以寫快寫慢不影響）
3. 逐筆比對：筆畫數 → 起筆位置 → 形狀（逐點平均距離）

| 結果 | 條件 |
| --- | --- |
| ◎ 正確 | 筆數、起筆、形狀都在門檻內 |
| △ 形狀偏了 | 筆順對，但某幾筆的平均距離超標（會指出第幾筆） |
| ✕ 筆順或筆畫數錯 | 筆數不對，或某一筆的起筆位置差太多 |

- **筆順錯會自然被抓到**：第 i 筆一律跟範本第 i 筆比，順序顛倒的話兩者差很遠
- **方向寫反**要兩端一起看才算——只比起點的話，筆順顛倒常會被誤報成「方向反了」
- 用「逐點平均距離」而不是 DTW：等距重新取樣已經把速度差異消掉了，
  DTW 剩下的彈性反而會把「形狀對但位置偏」也算成對，而位置正是田字格要教的
- 描寫模式**每寫完一筆就當場判那一筆並上色**；默寫模式要整格寫完才評，
  不然邊寫邊變紅等於偷偷給答案
- 範本的開關按鈕在兩種模式意思相反：默寫是「看答案」，描寫是「蓋住範本」
  （描寫時範本本來就在，給一顆「看答案」等於死按鈕）。蓋住之後逐筆判分照常，
  可以拿來當半默寫用

**門檻全部集中在 `src/lib/thresholds.ts`**，筆和手指兩套（手指接觸面積大、
控制差，門檻放寬）。現在給的是寬鬆的預設值——一開始就嚴格的話，寫對了卻一直
被判錯，人會直接放棄。實測之後往下調就改那個檔案。

### 手寫熟練度

跟辨識**完全分開**排複習（`state.write`，盒子和間隔規則一樣）。會認不代表會寫。

- 辨識到盒子 2 以上的字才會出默寫題
- 默寫題不佔複習題的額度，每天幾題在設定裡調
- 手寫**沒有補考**：重寫一次的成本比按一個選項高太多，答錯就交給間隔複習明天再來
- △（形狀偏了）算答對但不加點數——筆順對了就值得往上一格
...
```
[介紹 あ い う え お]  → [考這 5 個各 2 題 + 夾複習題，打散]
[介紹 か き く け こ]  → [考這 5 個各 2 題 + 夾複習題，打散]
[剩下的複習題和默寫題]
```

- **一次教幾個**在設定裡調（預設 5，可選 3／5／8／10）
- 每個新字在那一批**考兩次**：第一次是剛看完介紹卡的即時回想，第二次隔了幾題
  才回來，中間夾著複習題，才測得出是真的記住還是短期殘留
- 複習題**平均分進每一批**，不會全堆在最後——堆在最後的話前半段全是新字、
  後半段全是舊字，兩邊都很單調
- 複習題上限 20 題，跟新字分開算

首頁的「今天有 N 題」算的是實際要答的題數（新字 × 2 + 複習 + 默寫），
介紹卡不算題。
...
- 一個新字在同一批被考兩次，但**只有第一次計分**，第二次只給點數
